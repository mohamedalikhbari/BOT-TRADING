...
#property description "Derived from GoldIntradayEA v1.10: identical signals, tail-friendly exit management."
...
input int      InpEnsembleRsiMaxHoldingBars = 72;

input group "Position exits"
input int      InpRiskAtrPeriod = 20;
input double   InpInitialStopAtr = 1.75;
input double   InpTakeProfitR = 0.00;
input bool     InpUseAtrTrail = true;
input double   InpTrailAtr = 3.00;
input double   InpTrailActivationR = 1.00; // trailing attivo solo dopo X R di profitto
input int      InpMaxHoldingBars = 96;

input group "Breakeven"
input bool     InpUseBreakeven = false;
input double   InpBreakevenTriggerR = 1.50;
input double   InpBreakevenOffsetR = 0.20;
...
input ulong    InpMagic = 120262000;
...
// Peso della penalita di concentrazione nel criterio di ottimizzazione.
const double GOLD_TREND_CONCENTRATION_K = 5.0;
...
double g_tracked_stop_distance=0.0;
...
bool CloseOwnPosition(const ulong ticket,const string reason)
  {
   ...
   if(ok)
     {
      g_tracked_ticket=0;
      g_tracked_stop_distance=0.0;
     }
   if(!OptimizationQuiet())
      PrintFormat("GOLD_TREND_EXIT|time=%s|reason=%s|ok=%s|retcode=%u",
...
bool OpenDirection(const int direction,const double atr,const string detail)
  {
   ...
   if(volume<=0.0)
     {
      if(!OptimizationQuiet()) Print("GOLD_TREND_VETO|reason=volume");
      return false;
     }
   string comment=StringFormat("GoldTrend m%d",(int)InpMode);
   ...
   if(ok)
     {
      ulong opened_ticket;
      ENUM_POSITION_TYPE opened_type;
      datetime opened_time;
      if(SelectOwnPosition(opened_ticket,opened_type,opened_time))
        {
         g_tracked_ticket=opened_ticket;
         g_tracked_stop_distance=stop_distance;
        }
     }
   if(!OptimizationQuiet())
      PrintFormat("GOLD_TREND_ENTRY|time=%s|direction=%s|volume=%.2f|entry=%.2f|sl=%.2f|tp=%.2f|atr=%.2f|signal=%s|ok=%s|retcode=%u",
...
// Distanza di stop iniziale del ticket corrente. Al primo aggiornamento di una
// posizione non aperta in questa sessione la ricostruisce da POSITION_PRICE_OPEN
// e dallo stop loss ancora intatto.
double InitialStopDistance(const ulong ticket)
  {
   if(ticket==g_tracked_ticket && g_tracked_stop_distance>0.0)
      return g_tracked_stop_distance;
   double open_price=PositionGetDouble(POSITION_PRICE_OPEN);
   double stop=PositionGetDouble(POSITION_SL);
   double distance=(stop>0.0 ? MathAbs(open_price-stop) : 0.0);
   if(distance>0.0)
     {
      g_tracked_ticket=ticket;
      g_tracked_stop_distance=distance;
     }
   return distance;
  }

// Trailing ATR ad attivazione ritardata piu breakeven opzionale: fra i due
// candidati vince sempre lo stop loss piu favorevole alla posizione.
void ManageOpenPosition(const ulong ticket,const ENUM_POSITION_TYPE type,const MqlRates &bars[],const double atr)
  {
   if(!InpUseAtrTrail && !InpUseBreakeven) return;
   if(!PositionSelectByTicket(ticket)) return;
   double stop_distance=InitialStopDistance(ticket);
   ...
   double close_price=(direction>0 ? bid : ask);
   double progress_r=(double)direction*(close_price-entry)/stop_distance;
   ...
   if(InpUseAtrTrail && atr>0.0 && progress_r>=InpTrailActivationR)
     {
      candidate=NormalizeDouble(bars[0].close-(double)direction*InpTrailAtr*atr,digits);
      has_candidate=true;
     }
   if(InpUseBreakeven && progress_r>=InpBreakevenTriggerR)
     {
      double breakeven=NormalizeDouble(entry+(double)direction*InpBreakevenOffsetR*stop_distance,digits);
      if(!has_candidate || (direction>0 ? breakeven>candidate : breakeven<candidate))
        {
         candidate=breakeven;
         has_candidate=true;
        }
...
void EvaluateSignalBar()
  {
   ...
   if(has_position) ManageOpenPosition(ticket,type,bars,risk_atr);
   ...
   if(!OptimizationQuiet())
      PrintFormat("GOLD_TREND_SIGNAL|bar=%s|mode=%d|current=%d|desired=%d|close=%.2f|atr=%.2f|detail=%s",
...
void EnforceCircuitBreaker()
  {
   ...
   g_halted=true;
   if(!OptimizationQuiet()) Print("GOLD_TREND_HALT|reason=circuit_breaker");
  }
...
int OnInit()
  {
   ...
   if(_Symbol!="XAUUSD" || PeriodSeconds(InpSignalTimeframe)<=0 ||
      ...
      InpTrailAtr<=0.0 || InpTrailActivationR<0.0 || InpTrailActivationR>20.0 ||
      InpBreakevenTriggerR<=0.0 || InpBreakevenTriggerR>20.0 ||
      InpBreakevenOffsetR<0.0 || InpBreakevenOffsetR>=InpBreakevenTriggerR ||
   ...
   g_tracked_stop_distance=0.0;
   if(!OptimizationQuiet())
      PrintFormat("GOLD_TREND_INIT|mode=%d|timeframe=%s|leverage=1:%d|balance=%.2f|risk=%.3f|hold=%d|trail=%s|trail_r=%.2f|be=%s",
                  (int)InpMode,EnumToString(InpSignalTimeframe),
                  (int)AccountInfoInteger(ACCOUNT_LEVERAGE),
                  AccountInfoDouble(ACCOUNT_BALANCE),InpRiskPerTradePct,
                  InpMaxHoldingBars,(InpUseAtrTrail ? "true" : "false"),
                  InpTrailActivationR,(InpUseBreakeven ? "true" : "false"));
...
void OnDeinit(const int reason)
  {
   if(MQLInfoInteger(MQL_TESTER) && !OptimizationQuiet())
      PrintFormat("GOLD_TREND_SUMMARY|balance=%.2f|equity=%.2f|halted=%s",
                  AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY),
                  (g_halted ? "true" : "false"));
  }

// Criterio orientato alla robustezza: premia qualita e recupero invece del
// numero di trade, e penalizza esplicitamente i risultati sorretti da un solo
// trade eccezionale.
double OnTester()
  {
   ...
   double best_trade=TesterStatistics(STAT_MAX_PROFITTRADE);

   if(!MathIsValidNumber(profit)) profit=0.0;
   if(!MathIsValidNumber(trades) || trades<0.0) trades=0.0;
   if(!MathIsValidNumber(drawdown) || drawdown<0.0) drawdown=0.0;
   drawdown=MathMin(drawdown,100.0);
   if(!MathIsValidNumber(factor) || factor>10.0) factor=10.0;
   if(factor<0.0) factor=0.0;
   if(!MathIsValidNumber(recovery) || recovery<0.0) recovery=0.0;
   if(!MathIsValidNumber(gross_profit) || gross_profit<0.0) gross_profit=0.0;
   if(!MathIsValidNumber(best_trade) || best_trade<0.0) best_trade=0.0;

   if(trades<(double)InpMinTradesForCriterion)
      return -1000000.0+trades*1000.0+profit;
   if(profit<=0.0) return -100000.0+profit;

   double concentration=(gross_profit>0.0 ? best_trade/gross_profit : 1.0);
   concentration=MathMax(0.0,MathMin(1.0,concentration));

   double factor_component=MathMax(0.10,MathMin(4.0,factor));
   double recovery_component=MathMax(0.10,MathMin(20.0,recovery));
   double score=profit*factor_component*recovery_component;
   score/=(1.0+GOLD_TREND_CONCENTRATION_K*concentration);
   score/=(1.0+drawdown/10.0);
   if(!MathIsValidNumber(score)) return -100000.0;
   return score;
  }
