...
#property description "Research fork of frozen GoldIntradayEA. Never replaces the frozen advisor."
...
input bool     InpForceTrailingExit = false;
input double   InpTrailingTargetR = 2.00;
input double   InpTrailBreakevenFraction = 0.50;
input double   InpTrailPostTargetGapFraction = 0.10;
input double   InpTrailStepRisk = 0.05;
input double   InpTrailPartialCloseFraction = 0.80;
input double   InpTrailTrendGapRisk = 1.00;
...
input bool     InpUseDailyRiskThrottle = false;
input double   InpDailyThrottleStartPct = 0.01;
input double   InpDailyThrottleFullPct = 0.03;
input double   InpDailyThrottleMinRiskMultiplier = 0.10;
input bool     InpUsePreventiveRiskBudget = false;
input double   InpRiskBudgetSafetyFraction = 0.50;
...
double LinearRiskMultiplier(const double drawdown,const double start_at,
                            const double full_at,const double minimum)
  {
   if(drawdown<=start_at) return 1.0;
   if(drawdown>=full_at) return minimum;
   double progress=(drawdown-start_at)/(full_at-start_at);
...
double DrawdownRiskMultiplier()
  {
   ...
   if(InpUseDrawdownThrottle && g_peak_equity>0.0)
     {
      double global_dd=MathMax(0.0,1.0-equity/g_peak_equity);
      multiplier=MathMin(multiplier,LinearRiskMultiplier(global_dd,InpThrottleStartDDPct,
                                                          InpThrottleFullDDPct,
                                                          InpThrottleMinRiskMultiplier));
     }
   if(InpUseDailyRiskThrottle && g_daily_start_equity>0.0)
     {
      double daily_dd=MathMax(0.0,1.0-equity/g_daily_start_equity);
      multiplier=MathMin(multiplier,LinearRiskMultiplier(daily_dd,InpDailyThrottleStartPct,
                                                          InpDailyThrottleFullPct,
                                                          InpDailyThrottleMinRiskMultiplier));
     }
   return multiplier;
  }
...
double RiskSizedVolume(const int direction,const double entry,const double stop)
  {
   ...
   double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   double requested_loss=equity*InpRiskPerTradePct*DrawdownRiskMultiplier();
   if(InpUsePreventiveRiskBudget)
     {
      if(InpCircuitBreakerDDPct>0.0 && g_peak_equity>0.0)
        {
         double global_floor=g_peak_equity*(1.0-InpCircuitBreakerDDPct);
         requested_loss=MathMin(requested_loss,
                                MathMax(0.0,equity-global_floor)*InpRiskBudgetSafetyFraction);
        }
      if(InpDailyLossLimitPct>0.0 && g_daily_start_equity>0.0)
        {
         double daily_floor=g_daily_start_equity*(1.0-InpDailyLossLimitPct);
         requested_loss=MathMin(requested_loss,
                                MathMax(0.0,equity-daily_floor)*InpRiskBudgetSafetyFraction);
        }
     }
   if(requested_loss<=0.0) return 0.0;
   double requested=requested_loss/loss_per_lot;
   double minimum=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
   ...
   if(ok)
     {
      ...
      g_trailing_runner_active=false;
      if(InpMode==GOLD_H1_KELTNER_RSI_ENSEMBLE) g_active_ensemble_model=0;
     }
...
bool OpenDirection(const int direction,const double atr,const string detail)
  {
   ...
   double target=(target_distance>0.0 && !InpForceTrailingExit ?
                  NormalizeDouble(entry+(double)direction*target_distance,digits) : 0.0);
   ...
   if(ok)
     {
      g_active_initial_risk=stop_distance;
      g_trailing_runner_active=false;
     }
   if(!OptimizationQuiet())
...
void ApplyContinuousTrailingExit()
  {
   if(!InpForceTrailingExit || g_active_initial_risk<=0.0) return;
   ulong ticket;
   ENUM_POSITION_TYPE type;
   datetime opened_at;
   if(!SelectOwnPosition(ticket,type,opened_at) || !PositionSelectByTicket(ticket)) return;
   int direction=PositionDirection(type);
   double open_price=PositionGetDouble(POSITION_PRICE_OPEN);
   double current_sl=PositionGetDouble(POSITION_SL);
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   double market=(direction>0 ? bid : ask);
   double favorable=(double)direction*(market-open_price);
   double target_distance=InpTrailingTargetR*g_active_initial_risk;
   double trigger_distance=InpTrailBreakevenFraction*target_distance;
   if(favorable<trigger_distance) return;

   bool mean_reversion=(InpMode==GOLD_H1_KELTNER_RSI_ENSEMBLE && g_active_ensemble_model==2);
   if(mean_reversion && favorable>=target_distance && !g_trailing_runner_active)
     {
      ...
      double close_volume=NormalizeVolumeDown(volume*InpTrailPartialCloseFraction);
      if(step_volume>0.0 && volume-close_volume<minimum_volume)
         close_volume=NormalizeVolumeDown(volume-minimum_volume);
      if(close_volume>0.0 && close_volume<volume &&
         g_trade.PositionClosePartial(ticket,close_volume,InpDeviationPoints))
         g_trailing_runner_active=true;
     }

   double point=SymbolInfoDouble(_Symbol,SYMBOL_POINT);
   double minimum=(double)MathMax(SymbolInfoInteger(_Symbol,SYMBOL_TRADE_STOPS_LEVEL),
                                  SymbolInfoInteger(_Symbol,SYMBOL_TRADE_FREEZE_LEVEL))*point+2.0*point;
   int digits=(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS);
   double gap=trigger_distance;
   if(favorable>=target_distance)
      gap=(mean_reversion ? InpTrailPostTargetGapFraction*target_distance
                          : InpTrailTrendGapRisk*g_active_initial_risk);
   double candidate=market-(double)direction*gap;
   candidate=NormalizeDouble(candidate,digits);
   double step=InpTrailStepRisk*g_active_initial_risk;
   bool enough_step=(direction>0 ? candidate>=current_sl+step
                                 : current_sl<=0.0 || candidate<=current_sl-step);
   bool valid=(direction>0 ? candidate<bid-minimum : candidate>ask+minimum);
   if(enough_step && valid) g_trade.PositionModify(ticket,candidate,0.0);
  }
...
void SyncRiskDay()
  {
   ...
   if(today==g_risk_day_key) return;
   g_risk_day_key=today;
   g_daily_start_equity=AccountInfoDouble(ACCOUNT_EQUITY);
   g_daily_halted=false;
   if(!OptimizationQuiet())
      PrintFormat("GOLD_RESEARCH_DAY|day=%d|start_equity=%.2f",today,g_daily_start_equity);
  }
...
void EnforceRiskLimits()
  {
   ...
   if(!g_daily_halted && InpDailyLossLimitPct>0.0 && g_daily_start_equity>0.0 &&
      equity<=g_daily_start_equity*(1.0-InpDailyLossLimitPct))
     {
      ulong daily_ticket;
      ENUM_POSITION_TYPE daily_type;
      datetime daily_opened_at;
      if(SelectOwnPosition(daily_ticket,daily_type,daily_opened_at))
         CloseOwnPosition(daily_ticket,"DAILY_LOSS_LIMIT");
      g_daily_halted=true;
      if(!OptimizationQuiet())
         PrintFormat("GOLD_RESEARCH_HALT|reason=daily_loss|start=%.2f|equity=%.2f",
...
int OnInit()
  {
   ...
   if(_Symbol!="XAUUSD" || PeriodSeconds(InpSignalTimeframe)<=0 ||
      ...
      InpTrailAtr<=0.0 || InpTrailingTargetR<=0.0 || InpTrailBreakevenFraction<=0.0 ||
      InpTrailBreakevenFraction>1.0 || InpTrailPostTargetGapFraction<0.0 ||
      InpTrailPostTargetGapFraction>=1.0 || InpTrailStepRisk<=0.0 ||
      InpTrailPartialCloseFraction<=0.0 || InpTrailPartialCloseFraction>=1.0 ||
      InpTrailTrendGapRisk<=0.0 ||
      InpMaxHoldingBars<1 || InpBarExecutionDelayMinutes<0 ||
      ...
      InpDailyThrottleStartPct<0.0 || InpDailyThrottleFullPct<=InpDailyThrottleStartPct ||
      InpDailyThrottleFullPct>=1.0 || InpDailyThrottleMinRiskMultiplier<=0.0 ||
      InpDailyThrottleMinRiskMultiplier>1.0 || InpRiskBudgetSafetyFraction<=0.0 ||
      InpRiskBudgetSafetyFraction>1.0 ||
      InpCircuitBreakerDDPct<0.0 || InpCircuitBreakerDDPct>=1.0 ||
      InpDailyLossLimitPct<0.0 || InpDailyLossLimitPct>=1.0 ||
...
void OnTick()
  {
   ...
   if(!g_signal_pending || g_halted || g_daily_halted) return;
...
void OnDeinit(const int reason)
  {
   if(MQLInfoInteger(MQL_TESTER) && !OptimizationQuiet())
      PrintFormat("GOLD_RESEARCH_SUMMARY|balance=%.2f|equity=%.2f|halted=%s|daily_halted=%s",
                  AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY),
                  (g_halted ? "true" : "false"),(g_daily_halted ? "true" : "false"));
  }
...
double OnTester()
  {
   ...
   if(MQLInfoInteger(MQL_OPTIMIZATION))
      PrintFormat("GOLD_RESEARCH_PASS|risk=%.4f|profit=%.2f|trades=%.0f|equity_dd_pct=%.2f|pf=%.3f|sharpe=%.3f|halted=%s",
                  InpRiskPerTradePct,profit,trades,drawdown,factor,sharpe,(g_halted ? "true" : "false"));
