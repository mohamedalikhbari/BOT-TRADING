#property copyright "Gold Top-Down Sweep v1"
#property version   "1.00"
#property strict
#property description "H4/H1 structure, M15+M5 liquidity sweep, M1 BOS with FVG/iFVG/OB"
...
enum ENUM_H4_BIAS_MODE
  {
   ...
   H4_STRUCTURE_ONLY = 1,
   H1_EMA_PULLBACK_ONLY = 2
  };
input ENUM_H4_BIAS_MODE InpH4BiasMode = H4_STRUCTURE_ONLY;
input bool     InpRequireH1EmaPullback = true;
input double   InpH1PullbackAtrTolerance = 0.15;
enum ENUM_STRUCTURE_DIRECTION_MODE
  {
   STRUCTURE_RECENT_SWING_PERSISTENT = 0,
   STRUCTURE_STRICT_HIGHS_AND_LOWS = 1
  };
input ENUM_STRUCTURE_DIRECTION_MODE InpStructureDirectionMode = STRUCTURE_STRICT_HIGHS_AND_LOWS;
...
enum ENUM_M1_TRIGGER_MODE
  {
   M1_TRIGGER_STRICT_BOS = 0,
   M1_TRIGGER_ZONE_READY = 1,
   M1_TRIGGER_DISPLACEMENT = 2,
   M1_TRIGGER_STRUCTURE_BREAK = 3
  };
input ENUM_M1_TRIGGER_MODE InpM1TriggerMode = M1_TRIGGER_STRICT_BOS;
...
enum ENUM_TARGET_MODE
  {
   ...
   TARGET_M15_PREVIOUS_SWING = 2,
   TARGET_H1_PREVIOUS_SWING = 3
  };
input ENUM_TARGET_MODE InpTargetMode   = TARGET_M15_PREVIOUS_SWING;

enum ENUM_TRADING_WINDOW_MODE
  {
   TRADING_WINDOW_NY_OPEN = 0,
   TRADING_WINDOW_NY_0830_1100 = 1,
   TRADING_WINDOW_ALL_MARKET_HOURS = 2
  };
input ENUM_TRADING_WINDOW_MODE InpTradingWindowMode = TRADING_WINDOW_NY_0830_1100;

enum ENUM_M5_SWEEP_CONFIRM_MODE
  {
   M5_SWEEP_AGGREGATE_MOVEMENT = 0,
   M5_SWEEP_INDIVIDUAL_CANDLE = 1
  };
input ENUM_M5_SWEEP_CONFIRM_MODE InpM5SweepConfirmMode = M5_SWEEP_INDIVIDUAL_CANDLE;

enum ENUM_SWEEP_TIMEFRAME_MODE
  {
   SWEEP_M15_AND_M5 = 0,
   SWEEP_M5_ONLY = 1
  };
input ENUM_SWEEP_TIMEFRAME_MODE InpSweepTimeframeMode = SWEEP_M15_AND_M5;

enum ENUM_LIQUIDITY_SESSION_MODE
  {
   LIQUIDITY_PREVIOUS_NEW_YORK = 0,
   LIQUIDITY_PREVIOUS_LONDON = 1
  };
input ENUM_LIQUIDITY_SESSION_MODE InpLiquiditySessionMode = LIQUIDITY_PREVIOUS_NEW_YORK;
...
input ENUM_TRADE_DIRECTION_FILTER InpDirectionFilter = TRADE_BOTH_DIRECTIONS;
// Filtro di ricerca opzionale: richiede il massimo/minimo della sessione NY
// precedente oppure il massimo/minimo PRE-NY, coerente con la direzione.
input bool     InpRequirePnyOrPreNyPool = false;

input group "Strategia - pattern e trigger"
input int      InpMinSetupScore       = 0;
...
input double   InpMinWickRatioM5      = 0.35;
input int      InpSweepEventBarsM15   = 1;
input int      InpEntryTimeoutMinutes = 150;
input int      InpBiasMaxAgeH4        = 6;
input double   InpSlBufferATR         = 0.30;
input bool     InpUseFixedSlBuffer    = false;
input double   InpFixedSlBufferPrice  = 0.80;
enum ENUM_INITIAL_STOP_MODE
  {
   STOP_BEYOND_SWEEP = 0,
   STOP_STRUCTURAL_COMPOSITE = 1
  };
input ENUM_INITIAL_STOP_MODE InpInitialStopMode = STOP_BEYOND_SWEEP;
input double   InpMinSlDollars        = 0.00;
input double   InpMaxSlDollars        = 1000.00;
input bool     InpUseSlDistanceGuards = false;
input double   InpMinRMultiple        = 0.00;
input double   InpMaxRMultiple        = 0.00; // 0 = nessun limite massimo
input double   InpEntryRRFilterMin    = 0.00; // filtro sul primo target M15 selezionato
...
enum ENUM_EXIT_MODE
  {
   EXIT_SCALED = 0,
   EXIT_FLAT_2R = 1,
   EXIT_FIXED_M15_TARGET = 2,
   EXIT_DYNAMIC_M5_TRAIL = 3,
   EXIT_ADAPTIVE_M15_R = 4,
   EXIT_THEORETICAL_TARGET_TRAIL = 5
  };
input ENUM_EXIT_MODE InpExitMode      = EXIT_FIXED_M15_TARGET;
input double   InpFixedRMultiple      = 2.00;
input double   InpAdaptiveMinR        = 1.00;
input double   InpAdaptiveMaxR        = 3.00;
input double   InpDynamicBreakevenR   = 1.00;
input double   InpDynamicTrailATR     = 0.20;
...
input bool     InpUseMaxLotAbsolute   = false;
input bool     InpUsePortfolioRiskLimits = false;
input double   InpDailyLossLimitPct   = 0.05;
input double   InpMaxLossLimitPct     = 0.10;
input double   InpSafetyMargin        = 0.70;
input double   InpMaxSpread           = 1000.00;
input ulong    InpMagic               = 120260807;
input int      InpDeviationPoints     = 50;

input group "News e audit"
input bool     InpUseNewsFilter       = false;
input bool     InpRequireNewsCalendar = false;
...
Direction g_position_direction=DIR_NONE;
ZoneType g_position_zone_type=ZONE_NONE;
string g_position_sweep_levels="[]";
double g_position_planned_target=0.0;
int g_position_setup_score=0;
datetime g_position_force_close_at=0;
string g_position_force_close_source="";
...
datetime NyWindowBoundaryServer(const datetime server_time,const int hour,const int minute)
  {
   int y,m,d,h,mi;
   NewYorkDateFromServer(server_time,y,m,d,h,mi);
   return UtcToServer(NyLocalToUtc(y,m,d,hour,minute));
  }
...
bool LondonDstOnDate(const int year,const int month,const int day)
  {
   int start_day=LastSunday(year,3);
   int end_day=LastSunday(year,10);
...
int LondonUtcOffsetOnDate(const int year,const int month,const int day)
  {
   return LondonDstOnDate(year,month,day) ? 3600 : 0;
  }

datetime LondonLocalToUtc(const int year,const int month,const int day,const int hour,const int minute)
  {
   return MakeTime(year,month,day,hour,minute,0)-LondonUtcOffsetOnDate(year,month,day);
  }

bool SydneyDstOnDate(const int year,const int month,const int day)
  {
   int end_day=NthSunday(year,4,1);
   int start_day=NthSunday(year,10,1);
   if(month<4 || month>10) return true;
   if(month>4 && month<10) return false;
   if(month==4) return day<end_day;
   return day>=start_day;
  }

int SydneyUtcOffsetOnDate(const int year,const int month,const int day)
  {
   return SydneyDstOnDate(year,month,day) ? 11*3600 : 10*3600;
  }

datetime SydneyLocalToUtc(const int year,const int month,const int day,const int hour,const int minute)
  {
   datetime wall=MakeTime(year,month,day,hour,minute,0);
   return wall-SydneyUtcOffsetOnDate(year,month,day);
  }

void SydneyDateFromUtc(const datetime utc_time,int &year,int &month,int &day,int &hour,int &minute)
  {
   // Il primo passaggio trova la data locale; il secondo applica l'offset DST
   // relativo a quella data (Sydney e' spesso gia' nel giorno successivo).
   MqlDateTime guess;
   TimeToStruct(utc_time+10*3600,guess);
   datetime local_time=utc_time+SydneyUtcOffsetOnDate(guess.year,guess.mon,guess.day);
   MqlDateTime local;
   TimeToStruct(local_time,local);
   year=local.year;
   month=local.mon;
   day=local.day;
   hour=local.hour;
...
datetime NextSydneyPreOpenServer(const datetime entry_server_time)
  {
   datetime entry_utc=ServerToUtc(entry_server_time);
   int y,m,d,h,mi;
   SydneyDateFromUtc(entry_utc,y,m,d,h,mi);
   datetime deadline_utc=SydneyLocalToUtc(y,m,d,9,30);
   if(deadline_utc<=entry_utc)
     {
      MqlDateTime next_date;
      TimeToStruct(MakeTime(y,m,d)+24*3600,next_date);
      deadline_utc=SydneyLocalToUtc(next_date.year,next_date.mon,next_date.day,9,30);
     }
   return UtcToServer(deadline_utc);
  }

bool FridaySessionCloseSeconds(long &close_seconds)
  {
   close_seconds=-1;
   for(uint index=0;index<32;index++)
     {
      datetime session_from=0,session_to=0;
      if(!SymbolInfoSessionTrade(_Symbol,FRIDAY,index,session_from,session_to)) break;
      long start=(long)session_from;
      long finish=(long)session_to;
      if(finish<=start) finish+=24*3600;
      close_seconds=MathMax(close_seconds,finish);
     }
   return close_seconds>0;
  }

datetime FridayPreCloseServer(const datetime entry_server_time)
  {
   long close_seconds=0;
   if(!FridaySessionCloseSeconds(close_seconds)) return 0;
   MqlDateTime entry;
   TimeToStruct(entry_server_time,entry);
   return (datetime)((long)MakeTime(entry.year,entry.mon,entry.day)+close_seconds-2*3600L);
  }

datetime PositionForceCloseDeadline(const datetime entry_server_time,string &source)
  {
   if(InpTradingWindowMode==TRADING_WINDOW_ALL_MARKET_HOURS)
     {
      source="";
      return 0;
     }
   int y,m,d,h,mi;
   NewYorkDateFromServer(entry_server_time,y,m,d,h,mi);
   if(DayOfWeek(y,m,d)==5)
     {
      source="FRIDAY_WEEKLY_CLOSE_MINUS_2H";
      return FridayPreCloseServer(entry_server_time);
     }
   source="SYDNEY_OPEN_MINUS_30M";
   return NextSydneyPreOpenServer(entry_server_time);
  }

bool InTradingWindow(const datetime at)
  {
   if(InpTradingWindowMode==TRADING_WINDOW_ALL_MARKET_HOURS) return true;
   if(InpTradingWindowMode==TRADING_WINDOW_NY_0830_1100)
     {
      datetime start=NyWindowBoundaryServer(at,8,30);
      datetime end=NyWindowBoundaryServer(at,11,0);
...
datetime SetupDeadline(const datetime at)
  {
   if(InpTradingWindowMode==TRADING_WINDOW_ALL_MARKET_HOURS)
      return at+InpEntryTimeoutMinutes*60;
   if(InpTradingWindowMode==TRADING_WINDOW_NY_0830_1100)
      return NyWindowBoundaryServer(at,11,0);
   return NyOpenServer(at)+120*60;
  }

bool MustForceClose(const datetime at)
  {
   return g_position_force_close_at>0 && at>=g_position_force_close_at;
  }
...
bool RunSelfTests()
  {
   ...
   // Finestra 08:30-11:00 America/New_York, sempre ancorata al DST USA.
   datetime ny_winter_start=UtcToServer(NyLocalToUtc(2026,1,15,8,30));
   datetime ny_summer_end=UtcToServer(NyLocalToUtc(2026,7,15,11,0));
   MqlDateTime nws,nse;
   TimeToStruct(ny_winter_start,nws);
   TimeToStruct(ny_summer_end,nse);
   if(nws.hour!=15 || nws.min!=30 || nse.hour!=18 || nse.min!=0) ok=false;
   // Chiusura 30 minuti prima dell'ASX: 09:30 Australia/Sydney.
   datetime sydney_summer=UtcToServer(SydneyLocalToUtc(2026,1,15,9,30));
   datetime sydney_winter=UtcToServer(SydneyLocalToUtc(2026,7,15,9,30));
   MqlDateTime sys,syw;
   TimeToStruct(sydney_summer,sys);
   TimeToStruct(sydney_winter,syw);
   if(sys.hour!=0 || sys.min!=30 || syw.hour!=2 || syw.min!=30) ok=false;
   if(InpMinBreakATR<=0.0 || InpKH4<1 || InpKH1<1 || InpKM15<1 || InpKM5<1 || InpKM1<1)
      ok=false;
   PrintFormat("XAU_SELF_TEST|status=%s|winter=%s|summer=%s|mismatch=%s|sydney_summer=%s|sydney_winter=%s",
               ok ? "PASS" : "FAIL",
               ...
               TimeToString(sydney_summer,TIME_DATE|TIME_MINUTES),
               TimeToString(sydney_winter,TIME_DATE|TIME_MINUTES));
   return ok;
  }
...
Direction PersistentStructureDirection(const StructureSnapshot &snapshot,const double current_close)
  {
   if(!snapshot.has_last_high || !snapshot.has_prev_high ||
      !snapshot.has_last_low || !snapshot.has_prev_low) return DIR_NONE;

   Direction direction=DIR_NONE;
   // La relazione dello swing confermato piu' recente aggiorna il trend; negli
   // swing intermedi il trend precedente resta valido fino alla rottura protetta.
   if(snapshot.last_high.confirmed_at>=snapshot.last_low.confirmed_at)
     {
      if(snapshot.last_high.price>snapshot.prev_high.price) direction=DIR_LONG;
      else if(snapshot.last_high.price<snapshot.prev_high.price) direction=DIR_SHORT;
     }
   else
     {
      if(snapshot.last_low.price>snapshot.prev_low.price) direction=DIR_LONG;
      else if(snapshot.last_low.price<snapshot.prev_low.price) direction=DIR_SHORT;
     }

   // La chiusura oltre lo swing protetto prevale e rappresenta il cambio trend.
   if(current_close<snapshot.last_low.price) direction=DIR_SHORT;
   else if(current_close>snapshot.last_high.price) direction=DIR_LONG;
   if(snapshot.has_latest_event && snapshot.latest_event.kind==EVENT_CHOCH)
      direction=snapshot.latest_event.direction;
   return direction;
  }

Direction SelectedStructureDirection(const StructureSnapshot &snapshot,const double current_close)
  {
   if(InpStructureDirectionMode==STRUCTURE_RECENT_SWING_PERSISTENT)
      return PersistentStructureDirection(snapshot,current_close);
   if(!snapshot.has_last_high || !snapshot.has_prev_high ||
      !snapshot.has_last_low || !snapshot.has_prev_low) return DIR_NONE;

   Direction direction=DIR_NONE;
   bool higher_high=snapshot.last_high.price>snapshot.prev_high.price;
   bool higher_low=snapshot.last_low.price>snapshot.prev_low.price;
   bool lower_high=snapshot.last_high.price<snapshot.prev_high.price;
   bool lower_low=snapshot.last_low.price<snapshot.prev_low.price;
   if(higher_high && higher_low) direction=DIR_LONG;
   else if(lower_high && lower_low) direction=DIR_SHORT;

   // La rottura in chiusura del massimo/minimo strutturale protetto crea il
   // cambio di trend richiesto anche prima che maturi la nuova coppia di swing.
   if(current_close<snapshot.last_low.price) direction=DIR_SHORT;
   else if(current_close>snapshot.last_high.price) direction=DIR_LONG;
   if(snapshot.has_latest_event && snapshot.latest_event.kind==EVENT_CHOCH)
      direction=snapshot.latest_event.direction;
...
bool EvaluateBias(const datetime at,const double current_price,BiasResult &result)
  {
   EmptyBias(result,at,"indicator_unavailable");
   if(InpH4BiasMode==H1_EMA_PULLBACK_ONLY)
     {
      MqlRates h1_rates[];
      int h1_count=LoadClosedRates(PERIOD_H1,at,250,h1_rates);
      if(h1_count<InpEmaSlow*3)
        {
         result.reason="h1_ema_warmup";
         return false;
        }
      double h1_atr[];
      bool h1_quality[];
      double h1_fast[],h1_slow[];
      CalculateAtrAndQuality(h1_rates,h1_atr,h1_quality);
      CalculateEma(h1_rates,InpEmaFast,h1_fast);
      CalculateEma(h1_rates,InpEmaSlow,h1_slow);
      int h1_last=h1_count-1;
      result.evaluated_at=h1_rates[h1_last].time+PeriodSeconds(PERIOD_H1);
      result.ema_fast=h1_fast[h1_last];
      result.ema_slow=h1_slow[h1_last];
      if(!h1_quality[h1_last] || h1_atr[h1_last]<=0.0)
        {
         result.reason="h1_data_quality";
         return false;
        }
      if(h1_fast[h1_last]>h1_slow[h1_last]) result.direction=DIR_LONG;
      else if(h1_fast[h1_last]<h1_slow[h1_last]) result.direction=DIR_SHORT;
      else
        {
         result.reason="h1_ema_equal";
         return false;
        }
      if(InpRequireH1EmaPullback && MathAbs(current_price-h1_fast[h1_last])>
         InpH1PullbackAtrTolerance*h1_atr[h1_last])
        {
         result.direction=DIR_NONE;
         result.reason="h1_ema21_pullback";
         return false;
        }
      result.reason="ok_h1_ema_pullback";
      return true;
     }
   MqlRates rates[];
   ...
   Direction structure_direction=SelectedStructureDirection(structure,close);
   if(structure_direction==DIR_NONE)
     {
      result.direction=DIR_NONE;
      result.reason="h4_structure_unresolved";
      return false;
     }
   if(InpH4BiasMode==H4_STRUCTURE_ONLY)
      result.direction=structure_direction;
   else if(result.direction!=structure_direction)
     {
      result.direction=DIR_NONE;
...
bool H1Confirms(const datetime at,const Direction direction,const double price,string &reason)
  {
   ...
   Direction h1_direction=SelectedStructureDirection(snapshot,price);
...
string PoolName(const LiquidityPoolType type)
  {
   ...
   if(type==POOL_PNYH)
      return InpLiquiditySessionMode==LIQUIDITY_PREVIOUS_LONDON ? "PLH" : "PNYH";
   if(type==POOL_PNYL)
      return InpLiquiditySessionMode==LIQUIDITY_PREVIOUS_LONDON ? "PLL" : "PNYL";
...
int GetLiquidityLevels(const datetime at,const datetime pre_ny_as_of,LiquidityLevel &levels[])
  {
   ...
   // Sessione precedente selezionata, saltando weekend e giorni senza dati.
   for(int back=1;back<=7;back++)
     {
      ...
      datetime range_to=0;
      if(InpLiquiditySessionMode==LIQUIDITY_PREVIOUS_LONDON)
        {
         range_from=UtcToServer(LondonLocalToUtc(prior_date.year,prior_date.mon,prior_date.day,8,0));
         range_to=UtcToServer(LondonLocalToUtc(prior_date.year,prior_date.mon,prior_date.day,16,30));
        }
      else
        {
         range_from=UtcToServer(NyLocalToUtc(prior_date.year,prior_date.mon,prior_date.day,9,30));
         range_to=NyCloseServerForDate(prior_date.year,prior_date.mon,prior_date.day);
        }
      double pnyh,pnyl;
...
bool MovementSweepsLevel(const MqlRates &rates[],const double &atr[],const bool &quality[],
                         const int first_index,const int last_index,const LiquidityLevel &level,
                         const Direction direction,const double min_sweep_atr,
                         const double min_wick_ratio,int &breach_index,double &extreme,
                         double &movement_high,double &movement_low)
  {
   breach_index=-1;
   extreme=(direction==DIR_LONG ? DBL_MAX : -DBL_MAX);
   movement_high=-DBL_MAX;
   movement_low=DBL_MAX;
   if(first_index<0 || last_index<first_index || level.state==LIQUIDITY_SWEPT) return false;
   if(direction==DIR_LONG && !IsLowPool(level.type)) return false;
   if(direction==DIR_SHORT && !IsHighPool(level.type)) return false;

   for(int i=first_index;i<=last_index;i++)
     {
      if(!quality[i]) continue;
      bool breached=(direction==DIR_LONG ? rates[i].low<level.price : rates[i].high>level.price);
      if(breached) { breach_index=i; break; }
     }
   if(breach_index<0) return false;

   int extreme_index=breach_index;
   for(int i=breach_index;i<=last_index;i++)
     {
      if(!quality[i]) return false;
      movement_high=MathMax(movement_high,rates[i].high);
      movement_low=MathMin(movement_low,rates[i].low);
      if(direction==DIR_LONG && rates[i].low<extreme)
        {
         extreme=rates[i].low;
         extreme_index=i;
        }
      if(direction==DIR_SHORT && rates[i].high>extreme)
        {
         extreme=rates[i].high;
         extreme_index=i;
        }
     }
   double movement_range=movement_high-movement_low;
   if(movement_range<=0.0 || atr[extreme_index]<=0.0) return false;
   double penetration=(direction==DIR_LONG ? level.price-extreme : extreme-level.price);
   double rejection=(direction==DIR_LONG ? rates[last_index].close-extreme
                                         : extreme-rates[last_index].close);
   bool reclaimed=(direction==DIR_LONG ? rates[last_index].close>level.price
                                       : rates[last_index].close<level.price);
   return reclaimed && penetration>=min_sweep_atr*atr[extreme_index] &&
          rejection/movement_range>=min_wick_ratio;
  }
...
bool DetectSweep(const MqlRates &bar,const double atr,const datetime at,const Direction direction,SweepEvent &event)
  {
   ZeroMemory(event);
   MqlRates rates[];
   int n=LoadClosedRates(PERIOD_M15,at,MathMax(40,InpSweepEventBarsM15+20),rates);
   if(n<20) return false;
   double atr_values[];
   bool quality[];
   CalculateAtrAndQuality(rates,atr_values,quality);
   int last=n-1;
   int first=MathMax(0,last-InpSweepEventBarsM15+1);
   ...
   for(int i=0;i<ArraySize(levels);i++)
     {
      int breach_index;
      double level_extreme,movement_high,movement_low;
      if(!MovementSweepsLevel(rates,atr_values,quality,first,last,levels[i],direction,
                              InpMinSweepAtrM15,InpMinWickRatioM15,breach_index,
                              level_extreme,movement_high,movement_low)) continue;
      datetime breach_close=rates[breach_index].time+PeriodSeconds(PERIOD_M15);
      if(!InTradingWindow(breach_close)) continue;
      SetSweepPool(event,levels[i].type,levels[i].price);
      event.started_at=MathMin(event.started_at,rates[breach_index].time);
      event.high=MathMax(event.high,movement_high);
      event.low=MathMin(event.low,movement_low);
     }
   event.valid=event.level_count>0;
   event.direction=direction;
   event.occurred_at=at;
   event.extreme=(direction==DIR_LONG ? event.low : event.high);
...
bool ConfirmSweepOnM5(const SweepEvent &m15_event,SweepEvent &confirmed)
  {
   ...
   bool quality[];
   CalculateAtrAndQuality(rates,atr,quality);
   datetime start=m15_event.started_at;
   int first=-1,last=-1,constituent_count=0;
   for(int i=0;i<n;i++)
     {
      if(rates[i].time<start || rates[i].time>=m15_event.occurred_at) continue;
      if(first<0) first=i;
      last=i;
      constituent_count++;
     }
   int expected=(int)((m15_event.occurred_at-start)/PeriodSeconds(PERIOD_M5));
   if(first<0 || last<first || constituent_count!=expected) return false;

   confirmed.direction=m15_event.direction;
   confirmed.started_at=m15_event.started_at;
   ...
   for(int p=0;p<8;p++)
     {
      ...
      bool pool_confirmed=false;
      if(InpM5SweepConfirmMode==M5_SWEEP_INDIVIDUAL_CANDLE)
        {
        ...
        {
         int breach_index;
         double movement_high,movement_low;
         pool_confirmed=MovementSweepsLevel(rates,atr,quality,first,last,level,
                                            m15_event.direction,InpMinSweepAtrM5,
                                            InpMinWickRatioM5,breach_index,deepest,
                                            movement_high,movement_low);
        }
      if(!pool_confirmed) continue;
...
bool DetectSweepM5Only(const datetime at,const Direction direction,SweepEvent &event)
  {
   ZeroMemory(event);
   MqlRates bar;
   double atr;
   bool quality;
   if(!LatestClosedBar(PERIOD_M5,at,bar,atr,quality) || !quality) return false;
   LiquidityLevel levels[];
   GetLiquidityLevels(at,bar.time,levels);
   event.direction=direction;
   event.started_at=bar.time;
   event.occurred_at=at;
   event.high=bar.high;
   event.low=bar.low;
   event.close=bar.close;
   ...
   for(int i=0;i<ArraySize(levels);i++)
     {
      if(!CandleSweepsLevel(bar,atr,levels[i],direction,
                            InpMinSweepAtrM5,InpMinWickRatioM5)) continue;
      SetSweepPool(event,levels[i].type,levels[i].price);
     }
   event.valid=event.level_count>0;
   event.confirmed_m15=false;
   event.confirmed_m5=event.valid;
   return event.valid;
  }

bool PassResearchFilter(const SweepEvent &event,string &reason)
  {
   reason="";
   if(InpDirectionFilter==TRADE_LONG_ONLY && event.direction!=DIR_LONG)
     {
      reason="direction_long_only";
      return false;
     }
   if(InpDirectionFilter==TRADE_SHORT_ONLY && event.direction!=DIR_SHORT)
     {
      reason="direction_short_only";
      return false;
     }
   if(!InpRequirePnyOrPreNyPool) return true;

   ulong required_mask=0;
   if(event.direction==DIR_LONG)
      required_mask=PoolBit(POOL_PNYL)|PoolBit(POOL_PRE_NY_L);
   else if(event.direction==DIR_SHORT)
      required_mask=PoolBit(POOL_PNYH)|PoolBit(POOL_PRE_NY_H);
   if((event.level_mask&required_mask)==0)
     {
      reason="missing_pny_or_pre_ny_pool";
      return false;
     }
   return true;
  }
...
void ArmEntryFromReaction(const datetime at,const double low,const double high)
  {
   ...
   g_setup.confirmation_mode=(InpSweepTimeframeMode==SWEEP_M5_ONLY ? "M5_SWEEP" : "M15_M5_SWEEP");
   g_setup.candidate.valid=false;
   g_setup.candidate_base_score=-100000;
   Diag(InpSweepTimeframeMode==SWEEP_M5_ONLY ? "entry_armed_after_m5_sweep"
                                             : "entry_armed_after_m15_m5_sweep");
  }

void OnSignalClose(const datetime at)
  {
   if(g_setup.state!=SETUP_IDLE || PositionsTotal()>0 || OrdersTotal()>0) return;
   if(!InTradingWindow(at)) return;
   ENUM_TIMEFRAMES signal_tf=(InpSweepTimeframeMode==SWEEP_M5_ONLY ? PERIOD_M5 : PERIOD_M15);
   Diag(InpSweepTimeframeMode==SWEEP_M5_ONLY ? "m5_bars_in_window" : "m15_bars_in_window");
   MqlRates bar;
   double atr;
   bool quality;
   if(!LatestClosedBar(signal_tf,at,bar,atr,quality) || !quality)
     {
      Diag(InpSweepTimeframeMode==SWEEP_M5_ONLY ? "veto:m5:data_quality" : "veto:m15:data_quality");
      return;
     }
   ...
   BiasResult bias;
   if(!EvaluateBias(at,bar.close,bias))
     {
   ...
   if(InpRequireH1Alignment)
     {
      ...
      Diag("h1_filter_bypassed");
   Diag("h1_confirmed");
   SweepEvent sweep;
   bool sweep_found=(InpSweepTimeframeMode==SWEEP_M5_ONLY
                     ? DetectSweepM5Only(at,bias.direction,sweep)
                     : DetectSweep(bar,atr,at,bias.direction,sweep));
   if(!sweep_found)
     {
      Diag("veto:no_sweep");
      return;
   ...
   for(int p=0;p<8;p++)
     {
      LiquidityPoolType type=(LiquidityPoolType)p;
      if((sweep.level_mask&PoolBit(type))!=0)
         Diag((InpSweepTimeframeMode==SWEEP_M5_ONLY ? "sweep_m5_level:" : "sweep_m15_level:")+
              PoolName(type));
     }
   Diag("sweep_at:"+TimeToString(at,TIME_DATE|TIME_MINUTES));

   SweepEvent confirmed;
   if(InpSweepTimeframeMode==SWEEP_M5_ONLY)
      confirmed=sweep;
   else if(!ConfirmSweepOnM5(sweep,confirmed))
     {
   ...
   if(InpSweepTimeframeMode!=SWEEP_M5_ONLY && confirmed.level_count<sweep.level_count)
      Diag("sweep_m5_partial_confirmation");
   for(int p=0;p<8;p++)
     {
      LiquidityPoolType type=(LiquidityPoolType)p;
      if((confirmed.level_mask&PoolBit(type))!=0) Diag("sweep_level:"+PoolName(type));
     }
   // Lo stato del mercato va aggiornato anche quando un filtro di ricerca
   // decide di non trasformare lo sweep confermato in un setup.
   MarkSweepPools(at,confirmed);
   string research_filter_reason;
   if(!PassResearchFilter(confirmed,research_filter_reason))
     {
      Diag("veto:research_filter:"+research_filter_reason);
      return;
     }
   Diag(InpSweepTimeframeMode==SWEEP_M5_ONLY ? "sweep_confirmed_m5_only"
                                             : "sweep_confirmed_both_timeframes");
   ...
   PrintFormat("XAU_SWEEP|time=%s|direction=%s|levels=%s|prices=%s|extreme=%.2f|confirmed_m15=%s|confirmed_m5=true",
               ...
               confirmed.confirmed_m15 ? "true" : "false");
   // Non esiste alcun filtro Fibonacci.
   ArmEntryFromReaction(at,confirmed.extreme,bar.close);
  }
...
bool SelectTarget(const datetime at,const Direction direction,const double entry,
  ...
  {
   if(InpTargetMode==TARGET_M15_PREVIOUS_SWING || InpTargetMode==TARGET_H1_PREVIOUS_SWING)
     {
      ENUM_TIMEFRAMES target_tf=(InpTargetMode==TARGET_H1_PREVIOUS_SWING ? PERIOD_H1 : PERIOD_M15);
      int target_k=(InpTargetMode==TARGET_H1_PREVIOUS_SWING ? InpKH1 : InpKM15);
      int wanted_bars=(InpTargetMode==TARGET_H1_PREVIOUS_SWING ? 1400 : 900);
      StructureSnapshot target_snapshot;
      StructureEvent target_events[];
      SwingPoint target_swings[];
      if(!BuildStructure(target_tf,target_k,at,wanted_bars,target_snapshot,target_events,target_swings))
         return false;
      SwingKind wanted=(direction==DIR_LONG ? SWING_HIGH : SWING_LOW);
      // Primo swing confermato del timeframe target ancora davanti al prezzo.
      for(int i=ArraySize(target_swings)-1;i>=0;i--)
        {
         if(target_swings[i].confirmed_at>at || target_swings[i].kind!=wanted) continue;
         if((direction==DIR_LONG && target_swings[i].price>entry) ||
            (direction==DIR_SHORT && target_swings[i].price<entry))
           {
            double rr=MathAbs(target_swings[i].price-entry)/stop_distance;
            // Uno swing gia' quasi raggiunto non e' piu' un target utile: si usa
            // il successivo massimo/minimo precedente ancora valido.
            if(rr<InpMinRMultiple) continue;
            target=target_swings[i].price;
            return true;
           }
        }
      return false;
...
double NormalizeVolumeDown(const double raw)
  {
   ...
   double allowed_max=(InpUseMaxLotAbsolute ? MathMin(maximum,InpMaxLotAbsolute) : maximum);
...
void UpdateRiskDay(const datetime at)
  {
   if(g_loss_pause_until>0 && at>=g_loss_pause_until)
     {
      g_loss_pause_until=0;
      g_consecutive_losses=0;
      Diag("circuit_breaker:resumed_after_48h");
     }
   int y,m,d,h,mi;
   NewYorkDateFromServer(at,y,m,d,h,mi);
   int key=y*10000+m*100+d;
...
double EffectiveRiskFraction()
  {
   if(!InpUsePortfolioRiskLimits) return InpRiskPerTradePct;
...
bool PlaceMarketOrder(const datetime at,const Zone &zone,const MqlRates &bar)
  {
   ...
   double buffer=(InpUseFixedSlBuffer ? InpFixedSlBufferPrice : InpSlBufferATR*atr);
   // In entrambe le modalita' lo sweep rimane il limite minimo. La modalita'
   // composita amplia lo stop solo quando zona o swing M1 richiedono piu' spazio.
   double stop_anchor=g_setup.sweep.extreme;
   if(InpInitialStopMode==STOP_STRUCTURAL_COMPOSITE)
     {
      if(zone.valid)
        {
         if(direction==DIR_LONG) stop_anchor=MathMin(stop_anchor,zone.bottom);
         else stop_anchor=MathMax(stop_anchor,zone.top);
        }
      StructureSnapshot stop_structure;
      StructureEvent stop_events[];
      SwingPoint stop_swings[];
      if(BuildStructure(PERIOD_M1,InpKM1,at,500,stop_structure,stop_events,stop_swings))
        {
         if(direction==DIR_LONG && stop_structure.has_last_low)
            stop_anchor=MathMin(stop_anchor,stop_structure.last_low.price);
         else if(direction==DIR_SHORT && stop_structure.has_last_high)
            stop_anchor=MathMax(stop_anchor,stop_structure.last_high.price);
        }
      Diag("stop:structural_composite");
     }
   else
      Diag("stop:sweep");
   double stop=(direction==DIR_LONG ? stop_anchor-buffer : stop_anchor+buffer);
   ...
   if(InpUseSlDistanceGuards && stop_distance<min_distance)
     {
   ...
   if(InpUseSlDistanceGuards && stop_distance>InpMaxSlDollars)
     {
   ...
   double target=0.0;
   bool mobile_exit=(InpExitMode==EXIT_DYNAMIC_M5_TRAIL);
   bool fixed_r_exit=(InpExitMode==EXIT_FLAT_2R);
   bool adaptive_exit=(InpExitMode==EXIT_ADAPTIVE_M15_R);
   bool scaled_exit=(InpExitMode==EXIT_SCALED);
   bool target_trail_exit=(InpExitMode==EXIT_THEORETICAL_TARGET_TRAIL);
   if(mobile_exit || fixed_r_exit || scaled_exit)
      target=entry+(double)direction*(scaled_exit ? InpAdaptiveMaxR : InpFixedRMultiple)*stop_distance;
   else if(!SelectTarget(at,direction,entry,stop_distance,target))
     {
      if(adaptive_exit)
        {
         target=entry+(double)direction*InpFixedRMultiple*stop_distance;
         Diag("adaptive_target:fallback_fixed_r");
        }
      else
        {
   ...
   if(adaptive_exit)
     {
      double structural_r=MathAbs(target-entry)/stop_distance;
      double adaptive_r=MathMax(InpAdaptiveMinR,MathMin(InpAdaptiveMaxR,structural_r));
      target=entry+(double)direction*adaptive_r*stop_distance;
      Diag("adaptive_target:used");
     }
   double r_multiple=MathAbs(target-entry)/stop_distance;
   if(r_multiple<InpEntryRRFilterMin)
     {
      Diag("veto:order:entry_rr_min");
      PrintFormat("XAU_ORDER_VETO|time=%s|reason=entry_rr_min|entry=%.2f|stop=%.2f|target=%.2f|rr=%.3f|limit=%.2f",
                  TimeToString(at,TIME_DATE|TIME_MINUTES),entry,stop,target,r_multiple,
                  InpEntryRRFilterMin);
      ResetSetup("entry_rr_below_minimum");
      return false;
     }
   if(r_multiple<InpMinRMultiple)
     {
      ...
      ResetSetup("rr_below_minimum");
      return false;
     }
   if(InpMaxRMultiple>0.0 && r_multiple>InpMaxRMultiple)
     {
      Diag("veto:order:rr_max");
      PrintFormat("XAU_ORDER_VETO|time=%s|reason=rr_max|entry=%.2f|stop=%.2f|target=%.2f|rr=%.3f|limit=%.2f",
                  TimeToString(at,TIME_DATE|TIME_MINUTES),entry,stop,target,r_multiple,InpMaxRMultiple);
      ResetSetup("rr_above_maximum");
      return false;
     }
   ...
   double order_target=((mobile_exit || scaled_exit || target_trail_exit) ? 0.0 : target);
   string comment=StringFormat("GTDSv1_%I64d",(long)at);
   bool placed=false;
   if(direction==DIR_LONG)
      placed=g_trade.Buy(volume,_Symbol,0.0,stop,order_target,comment);
   else
      placed=g_trade.Sell(volume,_Symbol,0.0,stop,order_target,comment);
   if(!placed)
     {
      ...
      PrintFormat("XAU_ORDER_REJECTED|retcode=%u|description=%s|entry=%.2f|sl=%.2f|tp=%.2f|volume=%.2f",
                  g_trade.ResultRetcode(),g_trade.ResultRetcodeDescription(),entry,stop,order_target,volume);
      return false;
     }
   g_setup.pending_ticket=g_trade.ResultOrder();
   g_setup.pending_expiry=at+60;
   ...
   Diag("orders_placed");
   PrintFormat("GTDS_ORDER|time=%s|execution=MARKET_AFTER_M1_BOS|direction=%s|zone=%s|entry=%.2f|sl=%.2f|planned_tp=%.2f|server_tp=%.2f|volume=%.2f|score=%d|rr=%.2f|exit_mode=%d|stop_mode=%d",
               TimeToString(at,TIME_DATE|TIME_MINUTES),DirectionName(direction),ZoneTypeName(zone.type),
               entry,stop,target,order_target,volume,total_score,r_multiple,(int)InpExitMode,
               (int)InpInitialStopMode);
   return true;
  }
...
void OnM1Close(const datetime at)
  {
   ...
   bool choch_to_bias=EventAt(events,at,EVENT_CHOCH,direction);
   bool direct_swing_break=false;
   ...
   if(recent_count>=2 && atr>0.0)
     {
      ...
      if(direction==DIR_LONG && snapshot.has_last_high)
         direct_swing_break=(previous.close<=snapshot.last_high.price &&
                             bar.close>snapshot.last_high.price+InpMinBreakATR*atr);
      else if(direction==DIR_SHORT && snapshot.has_last_low)
         direct_swing_break=(previous.close>=snapshot.last_low.price &&
                             bar.close<snapshot.last_low.price-InpMinBreakATR*atr);
      displacement=(directional_break && body_atr>=InpMinM1DisplacementATR &&
                    body_atr<=InpMaxM1DisplacementATR);
     }
   bool structure_break=(bos || choch_to_bias || direct_swing_break);
   if(directional_break || structure_break)
      PrintFormat("XAU_TRIGGER_OBS|time=%s|direction=%s|body_atr=%.3f|previous_break=%s|bos=%s|choch_to_bias=%s|direct_swing_break=%s|zone=%s|zone_width_atr=%.3f|zone_age_min=%d|sweep_age_min=%d|levels=%s",
                  TimeToString(at,TIME_DATE|TIME_MINUTES),DirectionName(direction),body_atr,
                  (directional_break ? "true" : "false"),(bos ? "true" : "false"),
                  (choch_to_bias ? "true" : "false"),
                  (direct_swing_break ? "true" : "false"),
   ...
   if(InpM1TriggerMode==M1_TRIGGER_STRICT_BOS && bos && at>g_setup.candidate.created_at)
     {
      Diag("m1_trigger:strict_bos");
      triggered=true;
     }
   else if(InpM1TriggerMode==M1_TRIGGER_STRUCTURE_BREAK && structure_break &&
           at>=g_setup.candidate.created_at)
     {
      Diag("m1_trigger:structure_break");
      triggered=true;
     }
   ...
   if(!triggered) return;
   PlaceMarketOrder(at,g_setup.candidate,bar);
  }
...
void SyncPendingAndPosition(const datetime at)
  {
      ...
      if(g_setup.state!=SETUP_IN_POSITION)
        {
         ...
         g_position_mae=0.0;
         g_position_mfe=0.0;
         g_position_direction=g_setup.bias.direction;
         g_position_zone_type=g_setup.candidate.type;
         g_position_sweep_levels=SweepLevelNames(g_setup.sweep);
         g_position_planned_target=g_setup.target;
         g_position_setup_score=g_setup.setup_score;
         datetime position_opened_at=(datetime)PositionGetInteger(POSITION_TIME);
         g_position_force_close_at=PositionForceCloseDeadline(position_opened_at,
                                                              g_position_force_close_source);
         g_trades_today++;
         Diag("orders_filled");
         PrintFormat("XAU_FILL|time=%s|ticket=%I64u|entry=%.2f|volume=%.2f|force_close=%s|force_source=%s",
                     TimeToString(at,TIME_DATE|TIME_SECONDS),position_ticket,
                     g_position_initial_entry,g_position_initial_volume,
                     TimeToString(g_position_force_close_at,TIME_DATE|TIME_MINUTES),
                     g_position_force_close_source);
        }
      return;
     }
   if(g_setup.state==SETUP_IN_POSITION)
     {
      Diag("position_cycle_completed");
      ResetSetup("position_closed");
      g_position_stage=0;
      g_position_initial_volume=0.0;
      g_position_force_close_at=0;
      g_position_force_close_source="";
      // I valori di audit restano disponibili fino alla prossima apertura:
      // OnTradeTransaction puo' arrivare dopo questo sincronismo.
     }
...
bool ImprovePositionStop(const ulong ticket,const Direction direction,double candidate,const string reason)
  {
   if(!PositionSelectByTicket(ticket)) return false;
   ...
   double minimum_gap=(double)MathMax(stops_level,freeze_level)*point;
   if(direction==DIR_LONG)
     {
      candidate=MathMin(candidate,bid-minimum_gap);
      if(candidate<=0.0 || candidate>=bid ||
         (current_sl>0.0 && candidate<=current_sl+point*0.5)) return false;
     }
   else
     {
      candidate=MathMax(candidate,ask+minimum_gap);
      if(candidate<=ask ||
         (current_sl>0.0 && candidate>=current_sl-point*0.5)) return false;
     }
   candidate=NormalizeDouble(candidate,digits);
   if(!g_trade.PositionModify(ticket,candidate,PositionGetDouble(POSITION_TP)))
     {
      PrintFormat("XAU_DYNAMIC_STOP_REJECTED|reason=%s|retcode=%u|description=%s|candidate=%.2f|current=%.2f",
                  reason,g_trade.ResultRetcode(),g_trade.ResultRetcodeDescription(),candidate,current_sl);
      return false;
     }
   Diag("dynamic_stop:"+reason);
   PrintFormat("XAU_DYNAMIC_STOP|time=%s|reason=%s|old=%.2f|new=%.2f",
               TimeToString(TimeCurrent(),TIME_DATE|TIME_SECONDS),reason,current_sl,candidate);
   return true;
  }
...
void ManagePosition(const datetime at,const bool new_m5_bar)
  {
   ...
   bool risk_force=(InpUsePortfolioRiskLimits &&
                    daily_loss>=g_initial_balance*InpDailyLossLimitPct*InpSafetyMargin*0.60);
   bool session_force=MustForceClose(at);
   if(news_reason!="" || session_force || risk_force)
     {
      string reason=(news_reason!="" ? news_reason :
                     (risk_force ? "RISK_FORCE" : g_position_force_close_source));
   ...
   if(InpExitMode==EXIT_THEORETICAL_TARGET_TRAIL)
     {
      double target_distance=MathAbs(g_position_planned_target-g_position_initial_entry);
      if(target_distance<=0.0) return;
      if(signed_move>=0.50*target_distance)
        {
         double trail_distance=(signed_move>=target_distance ? 0.10*target_distance
                                                             : 0.50*target_distance);
         double protected_move=MathMax(0.0,signed_move-trail_distance);
         string trail_reason=(signed_move>=target_distance ? "target_90pct_trail"
                                                            : (g_position_stage==0 ? "target_half_breakeven"
                                                                                   : "target_half_trail"));
         if(ImprovePositionStop(ticket,direction,
                                g_position_initial_entry+(double)direction*protected_move,
                                trail_reason))
            g_position_stage=(signed_move>=target_distance ? 2 : 1);
        }
      return;
     }
   if(InpExitMode==EXIT_FIXED_M15_TARGET || InpExitMode==EXIT_ADAPTIVE_M15_R) return;
   if(InpExitMode==EXIT_FLAT_2R)
     {
      if(signed_move>=InpFixedRMultiple*one_r) g_trade.PositionClose(ticket,InpDeviationPoints);
      return;
     }
   if(InpExitMode==EXIT_DYNAMIC_M5_TRAIL)
     {
      if(g_position_stage==0 && signed_move>=InpDynamicBreakevenR*one_r)
        {
         double protected_price=g_position_initial_entry+(double)direction*spread;
         if(ImprovePositionStop(ticket,direction,protected_price,"breakeven"))
            g_position_stage=1;
        }
      if(g_position_stage>=1 && new_m5_bar && PositionSelectByTicket(ticket))
        {
         StructureSnapshot snapshot;
         StructureEvent events[];
         SwingPoint swings[];
         ...
         if(BuildStructure(PERIOD_M5,InpKM5,at,700,snapshot,events,swings) &&
            LatestClosedBar(PERIOD_M5,at,m5_bar,m5_atr,m5_quality) && m5_quality)
           {
            double candidate=0.0;
            if(direction==DIR_LONG && snapshot.has_last_low)
               candidate=snapshot.last_low.price-InpDynamicTrailATR*m5_atr;
            else if(direction==DIR_SHORT && snapshot.has_last_high)
               candidate=snapshot.last_high.price+InpDynamicTrailATR*m5_atr;
            if(candidate>0.0) ImprovePositionStop(ticket,direction,candidate,"m5_swing");
           }
        }
      return;
     }
   ...
   if(handle!=INVALID_HANDLE)
     {
      ...
      FileWrite(handle,"meta:news_filter",InpUseNewsFilter ? "true" : "false");
      ...
      FileWrite(handle,"meta:require_h1_alignment",InpRequireH1Alignment ? "true" : "false");
      FileWrite(handle,"meta:structure_mode",(int)InpStructureDirectionMode);
      FileWrite(handle,"meta:m5_confirm_mode",(int)InpM5SweepConfirmMode);
      FileWrite(handle,"meta:m1_trigger_mode",(int)InpM1TriggerMode);
      FileWrite(handle,"meta:trading_window_mode",(int)InpTradingWindowMode);
      FileWrite(handle,"meta:force_close_policy",
                InpTradingWindowMode==TRADING_WINDOW_ALL_MARKET_HOURS ? "none" :
                "sydney_0930_or_friday_broker_close_minus_2h");
      FileWrite(handle,"meta:initial_stop_mode",(int)InpInitialStopMode);
      FileWrite(handle,"meta:exit_mode",(int)InpExitMode);
...
int OnInit()
  {
   if(InpEntryTimeoutMinutes<=0 || InpMinWickRatioM5<=0.0 || InpMinWickRatioM5>1.0 ||
      ...
      InpMinRMultiple<0.0 || InpMaxRMultiple<0.0 || InpEntryRRFilterMin<0.0 ||
      (InpMaxRMultiple>0.0 && InpMaxRMultiple<InpMinRMultiple) ||
      (InpMaxRMultiple>0.0 && InpMaxRMultiple<InpEntryRRFilterMin) ||
      InpFixedRMultiple<=0.0 || InpAdaptiveMinR<=0.0 ||
      InpAdaptiveMaxR<InpAdaptiveMinR || InpDynamicBreakevenR<=0.0 ||
      InpDynamicTrailATR<0.0)
     {
   ...
   long friday_close_seconds=0;
   if(InpTradingWindowMode!=TRADING_WINDOW_ALL_MARKET_HOURS &&
      !FridaySessionCloseSeconds(friday_close_seconds))
     {
      Print("XAU_INIT|status=FAIL|reason=friday_session_unavailable");
      return INIT_FAILED;
     }
   ...
   g_news_available=(InpUseNewsFilter || InpRequireNewsCalendar) ? LoadNewsCalendar() : false;
   ...
   PrintFormat("GTDS_INIT|status=OK|symbol=%s|contract=%.2f|point=%.5f|digits=%d|fibonacci=false|news_filter=%s|news_available=%s|friday_close_server=%s|friday_force_server=%s",
               ...
               InpUseNewsFilter ? "true" : "false",g_news_available ? "true" : "false",
               friday_close_seconds>0 ? TimeToString((datetime)friday_close_seconds,TIME_MINUTES) : "n/a",
               friday_close_seconds>0 ? TimeToString((datetime)(friday_close_seconds-2*3600),TIME_MINUTES) : "n/a");
...
void OnTick()
  {
   ...
   // Prima il M1 e poi il timeframe sweep: una chiusura simultanea non anticipa il segnale HTF.
   if(new_m1) OnM1Close(m1_open);
   if(InpSweepTimeframeMode==SWEEP_M5_ONLY)
     {
      if(new_m5) OnSignalClose(m5_open);
     }
   else if(new_m15)
      OnSignalClose(m15_open);
  }
   ...
   if(entry==DEAL_ENTRY_OUT || entry==DEAL_ENTRY_OUT_BY)
     {
      ...
      double one_r=MathAbs(g_position_initial_entry-g_position_initial_stop);
      double mae_r=(one_r>0.0 ? g_position_mae/one_r : 0.0);
      double mfe_r=(one_r>0.0 ? g_position_mfe/one_r : 0.0);
      datetime closed_at=(datetime)HistoryDealGetInteger(trans.deal,DEAL_TIME);
      PrintFormat("XAU_TRADE_CLOSE|time=%s|deal=%I64u|position=%I64u|direction=%s|zone=%s|levels=%s|pnl=%.2f|mae=%.2f|mfe=%.2f|mae_r=%.3f|mfe_r=%.3f|entry=%.2f|initial_sl=%.2f|planned_tp=%.2f|score=%d|reason=%d",
                  TimeToString(closed_at,TIME_DATE|TIME_SECONDS),trans.deal,
                  (ulong)HistoryDealGetInteger(trans.deal,DEAL_POSITION_ID),
                  DirectionName(g_position_direction),ZoneTypeName(g_position_zone_type),
                  g_position_sweep_levels,pnl,g_position_mae,g_position_mfe,mae_r,mfe_r,
                  g_position_initial_entry,g_position_initial_stop,g_position_planned_target,
                  g_position_setup_score,
                  (int)HistoryDealGetInteger(trans.deal,DEAL_REASON));
      if(!InpUsePortfolioRiskLimits) return;
